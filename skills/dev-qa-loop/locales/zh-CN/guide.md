...
- 默认一个 QA 子 Agent。主 Agent 根据测试难度、所需能力、延迟和已知成本/预算，
  从宿主支持的模型及推理强度中自主选择。常规用例和 CI 摘要可用较轻量的模型；
  复杂集成、并发或安全行为可能需要更强推理。不臆测可用模型、能力或价格。
- 用户明确指定的模型或预算优先。指定模型不可用时说明限制并继续独立工作，不擅自替换。
  自主选择的模型不可用时，可另选适合的可用模型并说明变更，无需不必要的确认。
- 宿主支持时在派发参数中实际设置模型，并使用隔离上下文（有 `fork_turns` 时选择 `none`）
  和自包含交接。宿主不支持选择模型时继承默认值并说明限制。分别记录请求模型和宿主报告的
  实际模型；后者未公开时记为未知。仅在提示词里写模型名不会改变模型。
