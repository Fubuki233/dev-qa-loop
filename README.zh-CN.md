...
通过明确的代码快照交接，并跟进 GitHub Actions 检查。主 Agent 按任务选择适合的 QA 模型，不固定某个型号。
...
主 Agent 根据任务难度、所需能力、延迟和已知成本/预算，从宿主支持的模型及推理强度中自主选择。
用户明确指定的模型和预算优先；宿主不能选择模型时，继承默认模型并说明限制。
交接记录分别保存请求模型和宿主报告的实际模型；实际模型未公开时记为未知。
...
```text
$dev-qa-loop 实现分页查询 API，主模型开发，自主选择合适的 QA 模型并行写测试，按快照验证并跟进 PR CI。
```
